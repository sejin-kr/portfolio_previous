...
gulp.task('imagemin', () => {
    return new Promise(resolve => {
        gulp
            .src([PATH.ASSETS.IMAGES + '/**/*.{gif,jpg,png,svg,pdf,cur,webp}', PATH.ASSETS.IMAGES + '/*.{gif,jpg,png,svg,pdf,cur,webp}'])
...
gulp.task('watch', () => {
    return new Promise(resolve => {
        ...
        gulp.watch([PATH.ASSETS.STYLE + "/*.{scss,css}", PATH.ASSETS.STYLE + "/**/*.{scss,css}"], gulp.series(['scss:compile']));
        gulp.watch([PATH.ASSETS.JS + "/**/*.js", PATH.ASSETS.JS + "/*.js"], gulp.series(['script:concat']));
